...
/* NOTES                                                           Divide by 8
 * to get the byte        Get the actual bit*/
#define STIPBIT(_a)                                                       \
    (zbstipplepattern                                                     \
...
#define ZCMP(z, zpix, _a, c) \
    (((!zbdt) || (z >= zpix)) STIPTEST(_a) NODRAWTEST(c))
...
#define PUT_PIXEL(_a)                                                          \
    {                                                                          \
        {                                                                      \
            register GLuint zz = z >> ZB_POINT_Z_FRAC_BITS;                    \
            if (ZCMPSIMP(zz, pz[_a], _a, 0)) {                                 \
                /*pp[_a] = RGB_MIX_FUNC(or1, og1, ob1, TEXTURE_SAMPLE(texture, \
                 * s, t));*/                                                   \
                TGL_BLEND_FUNC(RGB_MIX_FUNC(or1, og1, ob1,                     \
                                            (TEXTURE_SAMPLE(texture, s, t))),  \
                               (pp[_a]));                                      \
                if (zbdw)                                                      \
                    pz[_a] = zz;                                               \
            }                                                                  \
...
#define PUT_PIXEL(_a)                                                 \
    {                                                                 \
        {                                                             \
            register GLuint zz = z >> ZB_POINT_Z_FRAC_BITS;           \
            if (ZCMPSIMP(zz, pz[_a], _a, 0)) {                        \
                pp[_a] = RGB_MIX_FUNC(or1, og1, ob1,                  \
                                      TEXTURE_SAMPLE(texture, s, t)); \
                if (zbdw)                                             \
                    pz[_a] = zz;                                      \
            }                                                         \
