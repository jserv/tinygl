...
void draw()
{
    if (mb2) {  // Use an additional input to move gui elements for testing-
                // right click moves the button.
        tbcoords.d[0] = omg_cursorpos[0];
        tbcoords.d[1] = omg_cursorpos[1];
        haveclicked = 0;
    }

    if (omg_textbox(0.01, 0, "\nEntry 1\n", 24, 1, 0.4, 0.2, 0xFFFFFF, 0) &&
        omg_cb == 2)
        puts("Entry 1");
    ...
    if (omg_textbox(slidcoords.d[0], slidcoords.d[1], "\n Slider \n", 16, 1,
                    0.4, 0.3, 0xFFFFFF, haveclicked ? 0xFF0000 : 0x00) &&
        omg_cb == 1) {
        slidersliding = 1;
...
int main(int argc, char **argv)
{
    ...
    while (isRunning) {
        ...
        while (SDL_PollEvent(&ev))
            events(&ev);
        if (using_cursorkeys)
            omg_update_keycursor(dirbstates[0], dirbstates[1], dirbstates[2],
                                 dirbstates[3], mb);
        else {
