...
static inline mat4 perspective(f_ fov, f_ aspect, f_ near, f_ far)
{
    ...
    ret.d[0*4+3]=0; ret.d[1*4+0]=0; 		ret.d[1*4+1]=yScale;ret.d[1*4+2]=0;
        ...
        for (int j = 0; j < 4; j++)
            ret.d[i * 4 + j] =
                dotv4(/*j is the ROW of the target, i is the COLUMN.*/
                      getrow(a,
                             j), /*we retrieve the same ROW as our ROW INDEX.*/
                      getcol(b, i) /*we retrieve the same COLUMN as our COLUMN
                                      INDEX.*/
                );
    return ret;
}
...
{
    return (vec4) {
        .d[0] = in.d[0], .d[1] = in.d[1], .d[2] = in.d[2], .d[3] = w};
}
...
{ /*Just points along the minimum separating axis, Nothing fancy.*/
