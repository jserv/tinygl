...
void rotateCamera()
{
    ...
    right.d[1] = 0;
    // vec3 forward = glm::vec3(glm::normalize(glm::rotate(angle, right) *
...
int main(int argc, char **argv)
{
    ...
    if (screen->format->Rmask != 0x00FF0000 ||
        ...
        needsRGBAFix = 1;
        printf(
            "\nYour screen is using an RGBA output different than this library "
            "expects.");
        printf(
    ...
    // initScene();
    {
        ...
        if (!omodel.positions) {
            ...
            if (!testingModelArrays) {
                ...
                if(ModelArray.normals)glNormalPointer(GL_FLOAT,0,ModelArray.normals);
                //Must be 3!
                ...
                */
                ...
                /*Done multiple times to test and make sure that data isn't
                 * leaked*/
                puts("\nTesting glBufferData data integrity\n");
                glBufferData(GL_ARRAY_BUFFER,
                             sizeof(GLfloat) * 3 * ModelArray.npoints,
                             ModelArray.points, GL_STATIC_DRAW);
                glBufferData(GL_ARRAY_BUFFER,
                             sizeof(GLfloat) * 3 * ModelArray.npoints,
                             ModelArray.points, GL_STATIC_DRAW);
                glBufferData(GL_ARRAY_BUFFER,
                ...
                glBindBufferAsArray(GL_VERTEX_BUFFER, buffers[0], GL_FLOAT, 3,
                                    0);
                if (ModelArray.colors) {
                    glBindBuffer(GL_ARRAY_BUFFER, buffers[1]);
                    glBufferData(GL_ARRAY_BUFFER,
                                 sizeof(GLfloat) * 3 * ModelArray.npoints,
                                 ModelArray.colors, GL_STATIC_DRAW);
                    glBindBufferAsArray(GL_COLOR_BUFFER, buffers[1], GL_FLOAT,
                                        3, 0);
                }
                if (ModelArray.normals) {
                    ...
                    glBufferData(GL_ARRAY_BUFFER,
                                 sizeof(GLfloat) * 3 * ModelArray.npoints,
                                 ModelArray.normals, GL_STATIC_DRAW);
                    glBindBufferAsArray(GL_NORMAL_BUFFER, buffers[2], GL_FLOAT,
                                        3, 0);
                }
                if (ModelArray.texcoords) {
                    ...
                    glBufferData(GL_ARRAY_BUFFER,
                                 ...
                                 ModelArray.texcoords, GL_STATIC_DRAW);
                    glBindBufferAsArray(GL_TEXTURE_COORD_BUFFER, buffers[3],
                                        GL_FLOAT, 2, 0);
                }
            }
            freemodel(&m);
...
#define HEIGHT winSizeY
        ...
        // right.d[1] = 0;
        matrix = (lookAt(
            campos, addv3(campos, camforw),
            camup));  // Using right vector to correct for screen rotation.
        glLoadMatrixf(matrix.d);
        if (wasdstate[0])
        ...
        if (!dlExists) {
            if (!testingModelArrays) {
                ...
                for (unsigned int i = 0; i < count; i++) {
                    ...
                    mat4 total_translation =
                        multm4(multm4(horiz_translation, vert_translation),
                               ztranslation);
                    glMultMatrixf(total_translation.d);
                    // glTranslatef((float)(i % 10) * 8.0, (float)(i / 10)
                    // * 8.0, -10);
                    glBegin(GL_TRIANGLES);
            ...
            for (unsigned int i = 0; i < count; i++) {
                ...
                glMultMatrixf(total_translation.d);
                // glTranslatef((float)(i % 10) * 8.0, (float)(i / 10) * 8.0,
                // -10);
