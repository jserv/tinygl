...
{
    char *d = NULL;
    d = STRUTIL_ALLOC(strlen(s1) + strlen(s2) + 1);
    if (d) {
        strcpy(d, s1);
        strcat(d, s2);
    }
...
{
    unsigned long lenpre = strlen(pre), lenstr = strlen(str);
...
static unsigned long read_until_terminator(FILE *f,
                                           char *buf,
...
static char *read_until_terminator_alloced(FILE *f,
...
*/
static char *strencodealloc(const char *inbuf,
                            const char **tokens,
                            ...
                            char tokmark)
{
    ...
    for (i = 0; i < lenin; i++) {
        ...
        for (t = 0; t < ntokens; t++) /*t- the token we are processing.*/
            if (strprefix(tokens[t * 2 + 1],
                          inbuf + i)) { /*Matched at least one*/
                unsigned long h, curtoklen, howmany = 1;
                curtoklen =
                    strlen(tokens[t * 2 + 1]); /*Length of the current token we
                                                  are counting*/
...
static char *strdecodealloc(char *inbuf)
{
    ...
    while (c != esc) {
        ...
        while (isalpha(c)) {
            c_str[0] = c;
            tokens[(ntokens - 1) * 2] =
                strcatallocf1(tokens[(ntokens - 1) * 2], c_str);
            {
                if (i <= lenin)
                    c = inbuf[i++];
                else {
        ...
        for (vv = 0; vv < l; vv++) {
            ...
            c_str[0] = c;
            tokens[(ntokens - 1) * 2 + 1] =
                strcatallocf1(tokens[(ntokens - 1) * 2 + 1], c_str);
        }
        {
            if (i <= lenin)
                c = inbuf[i++];
    ...
    if (tokens) {
        unsigned long j;
        for (j = 0; j < ntokens; j++) {
            STRUTIL_FREE(tokens[j * 2]);
            STRUTIL_FREE(tokens[j * 2 + 1]);
        }
        STRUTIL_FREE(tokens);
    }
...
static char *str_repl_alloc(char *text, char *subtext, char *replacement)
{
    ...
    if (bruh == -1)
        return (strcatalloc("", text)); /*This is already proper.*/
...
static strll *consume_until(strll *current_node,
                            const char *find_me,
                            const char delete_findable)
{
    long loc;
    strll *right_old;
    char *text_old;
    ...
    current_node->right->right = right_old;
    text_old = current_node->text;
    current_node->text = str_null_terminated_alloc(
        text_old, loc + (delete_findable ? strlen(find_me) : 0));
    current_node->right->text =
        strcatalloc(text_old + loc + strlen(find_me), "");
    STRUTIL_FREE(text_old);
    return current_node->right;
}
...
static strll tokenize(char *alloced_text, const char *token)
{
    ...
    while (current_token_location > -1) {
        char *temp =
            strcatalloc(alloced_text + current_token_location + len_token, "");
        current->text =
            str_null_terminated_alloc(alloced_text, current_token_location);
...
static strll *parse_matched(strll *current_node, const char *tl, const char *tr)
{
    strll *child_old;
    strll *right_old;
    long current_tl_location;
    long counter = 1;
    long current_tr_location;
    long start_tl_location;
    long len_tl;
    long len_tr;
    len_tl = strlen(tl);
    len_tr = strlen(tr);
    ...
    if (current_tr_location > -1 && current_tr_location < current_tl_location) {
        printf("\n<SYNTAX ERROR> %s before %s\n", tr, tl);
        exit(1);
    }
    if (current_tr_location == -1 && current_tl_location != -1) {
    ...
    /*Inch along, incrementing on*/
    {
        ...
        while (counter > 0) {
            ...
            if (current_tl_location != -1 &&
                ...
                metaproc += current_tl_location + len_tl;
                counter++;
            } else {
                metaproc += current_tr_location + len_tr;
                counter--;
        ...
        if (counter < 0) {
            printf(
                "\n<INTERNAL ERROR> Counter for %s,%s group somehow went "
                "negative.\n",
                tl, tr);
            exit(1);
        }
        current_node->child->text =
            str_null_terminated_alloc(begin, off - len_tr);
        {
            char *text_old = current_node->text;
            current_node->text =
                str_null_terminated_alloc(text_old, start_tl_location);
