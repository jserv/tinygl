...
void gl_transform_to_viewport(GLContext *c, GLVertex *v)
{
	...
	{
		v->zp.s = (int)(v->tex_coord.X * (ZB_POINT_UV_MAX - ZB_POINT_UV_MIN) + ZB_POINT_UV_MIN);
		v->zp.t = (int)(v->tex_coord.Y * (ZB_POINT_UV_MAX - ZB_POINT_UV_MIN) + ZB_POINT_UV_MIN);
	}
}
...
void clip_interpolate(GLVertex *q, GLVertex *p0, GLVertex *p1, float t)
{
...
int ClipLine1(float denom, float num, float *tmin, float *tmax)
{
	float t;
...
void gl_draw_line(GLContext *c, GLVertex *p1, GLVertex *p2)
{
	...
	else
	{
		...
		if ( ClipLine1(dx+dw,-x1-w1,&tmin,&tmax) &&
			 ...
			 ClipLine1(-dz+dw,z1-w1,&tmin,&tmax) )
		{
			clip_interpolate(&q1,p1,p2,tmin);
			clip_interpolate(&q2,p1,p2,tmax);
...
void updateTmp(GLContext *c, GLVertex *q, GLVertex *p0, GLVertex *p1, float t)
{
...
void gl_draw_triangle_fill(GLContext *c, GLVertex *p0, GLVertex *p1, GLVertex *p2)
{
	...
	if ( c->texture.enabled_2d )
	{
		if ( c->texture.current->images[0].pixmap == NULL )
		{
			return;
		}
#ifdef PROFILE
...
#endif
		ZB_setTexture(c->zb,&c->texture.current->images[0]);
