...
#endif
	default:
		tgl_fatal_error("%s this format conversion is not implemented",__FUNCTION__);
	}
}
...
void ZB_copyBufferRGB32(ZBuffer * zb, void *buf, int linesize)
{
	unsigned char* source = (unsigned char*) zb->pbuf;
	unsigned int* destination;
	unsigned int _pixel = 0;

	for(int y=0;y<zb->ysize;y++)
	{
		destination = (unsigned int*) (((unsigned char*) buf) + linesize*y);
		for(int x=0;x<zb->xsize;x++)
		{
			_pixel = (unsigned int) *source++;
			_pixel <<= 8;
			_pixel |= *source++;
			_pixel <<= 8;
			_pixel |= *source++;
...
#ifdef TGL_FEATURE_32_BITS
	case ZB_MODE_RGBA:
		ZB_copyBufferRGB32(zb, buf, linesize);
		break;
#endif
	default:
		tgl_fatal_error("%s this format conversion is not implemented",__FUNCTION__);
